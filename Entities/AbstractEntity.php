...
abstract class AbstractEntity {
	...
	public function __set($name, $value){
		$propertyName = str_replace(FcsXmlParser::$charsToRemove, "", $name);
	...
	/**
	 * Extracts each property name and value for the current object and push it in the array.
	 * 
	 * @param 	array|string &$notIncluded
	 */
	public function convertToSingleFullArray(&$refArray=array()) {
		$propertiesNameList = $this->getPropertiesNameList();
		foreach ($propertiesNameList as $propertyName){
			$method = 'get' . ucfirst($propertyName);
			$value = $this->$method();
			if(is_object($value)){
				$value->convertToSingleFullArray($refArray);
			} else {
