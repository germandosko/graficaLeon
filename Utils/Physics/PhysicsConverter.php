...
class PhysicsConverter
{
	 ...
	 * @static
	 */
	public static function changeLenghtUnit($value, $from, $to) {
		...
		switch($from){
			...
			case PhysicsUnits::GEO_COORDS :
				...
				break;
			default:
				throw new Exception('Unable to recognize Lenght Measurement Unit.(PhisicsConverter::changeLenghtUnit)');
		}
	...
	public static function changeVelocityUnit($value, $from, $to) {
		...
		switch($from){
			...
			case PhysicsUnits::KM_PER_HOUR :
				...
				$mtsPerSec = self::changeLenghtUnit($kmPerSec, PhysicsUnits::KILOMETERS, PhysicsUnits::METERS);
				break;
			case PhysicsUnits::GEO_COORDS_PER_SECOND :
				$mtsPerSec = self::changeLenghtUnit($value, PhysicsUnits::GEO_COORDS, PhysicsUnits::METERS);
				break;
			default:
		...
		switch($to){
			case PhysicsUnits::METERS_PER_SECOND :
				...
				break;
			case PhysicsUnits::KM_PER_HOUR :
				$kmPerSec = self::changeLenghtUnit($mtsPerSec, PhysicsUnits::METERS, PhysicsUnits::KILOMETERS);
				$converted = $kmPerSec * self::changeTimeUnit(1, PhysicsUnits::HOURS, PhysicsUnits::SECONDS);
				break;
			case PhysicsUnits::GEO_COORDS_PER_SECOND :
				$converted = self::changeLenghtUnit($mtsPerSec, PhysicsUnits::METERS, PhysicsUnits::GEO_COORDS);
				break;
			default:
				throw new Exception('Unable to recognize Velocity Measurement Unit.(PhisicsConverter::changeVelocityUnit)');
		}
