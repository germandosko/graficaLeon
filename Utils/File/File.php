...
class File{
	...
	public static function GetInfo($filePath){
		if(file_exists($filePath)){
			if(is_readable($filePath)){
				...
				$info['parent_folder'] = substr($filePath, 0, ($slashPosition+1));
	...
	public static function GetCreationDate($filePath){
		if(file_exists($filePath)){
			if(is_readable($filePath)){
				return date('Y-m-d H:m:s', filectime($filePath));
			} else {
	...
	public static function GetModificationDate($filePath){
		if(file_exists($filePath)){
			if(is_readable($filePath)){
				return date('Y-m-d H:m:s', filemtime($filePath));
			} else {
	...
	public static function GetLastAccessDate($filePath){
		if(file_exists($filePath)){
			if(is_readable($filePath)){
				return date('Y-m-d H:m:s', fileatime($filePath));
			} else {
