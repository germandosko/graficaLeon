...
class ExcelXml{
	...
	private function createXmlHeader(){
		...
		$this->addXmlLine($this->xmlHeader, '<Created>'.date("Y-m-d").'T'.date("H:i:s").'Z</Created>', 2);
	...
	private function addHeaderRow($columns){
		...
		foreach($columns as $column){
			$columnText = '<Column ss:StyleID="' . $column['style'] . '" ss:AutoFitWidth="' . $column['auto'] . '" ss:Width="' . $column['width'] . '"/>';
