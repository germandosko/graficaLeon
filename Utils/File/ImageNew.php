...
class ImageNew{
	...
	public function __construct($path=null){
		...
		$this->path = $path;
		$this->name = FileNew::GetNameWithoutExtension($path);
		$this->width = $info[0];
		$this->height = $info[1];
		$this->bits = $info['bits'];
		$this->mime = $info['mime'];
	 ...
	 */
	public function addTextWatermark($text, $ttfFile, $fontSize=20, $angle=0, $color=array(0,0,0,127), $position=ImageNew::POSITION_BOTTOM_RIGHT, $opacity=100){
		...
		$color = imagecolorallocate($textImage, $color[0], $color[1], $color[2]);
		...
		imagettftext($textImage, $fontSize, $angle, $x, $y, $color, $ttfFile, $text);
		//Adding watermark
		$x = $marginX = 10;
		$y = $marginY = 10;
	 ...
	 */
	public function addImageWatermark($watermark, $position=Image::POSITION_BOTTOM_RIGHT, $opacity=10){
		$x = $marginX = 10;
		$y = $marginY = 10;
		if(($position == self::POSITION_TOP_CENTER) || ($position == self::POSITION_MIDDLE_CENTER) || ($position == self::POSITION_BOTTOM_CENTER)){
	...
	public function exportTo($mimeType=null, $folderPath=null, $imageName=null, $quality=100){
		...
		if(empty($folderPath)){
			$folderPath = FileNew::GetParentFolderPath($this->path, true) . 'GdImages/';
			if(!is_dir($folderPath)){
				mkdir($folderPath);
			}
		}
		if(!empty($imageName)){			
			$imageName = $this->name . $imageName;
		} else {
			$imageName = $this->name;
		}
		switch ($mimeType) {
